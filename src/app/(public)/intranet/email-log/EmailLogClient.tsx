...
export function EmailLogClient() {
  const { enabled, authenticated, isLoading: isLoadingSession } = useStaffSession();
    ...
    async function load() {
      try {
        ...
        const [entriesRes, configRes] = await Promise.all([
          fetch(buildClientApiUrl('/api/email-log'), { cache: 'no-store', credentials: 'include' }),
          fetch(buildClientApiUrl('/api/email-config'), {
            cache: 'no-store',
            credentials: 'include',
          }),
        ]);
  ...
  }, [authenticated, enabled, isLoadingSession]);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {isLoadingSession ? (
          <section className="rounded-lg border border-gray-200 bg-white p-6 text-center text-sm text-gray-600 shadow-sm">
            Checking Microsoft staff session...
          </section>
        ) : !enabled ? (
          <section className="rounded-lg border border-amber-200 bg-amber-50 p-6 text-center text-sm text-amber-800">
            Microsoft staff sign-in is not configured yet.
          </section>
        ) : !authenticated ? (
          <section className="rounded-lg border border-gray-200 bg-white p-6 text-center shadow-sm">
            <p className="mb-4 text-sm text-gray-600">Sign in with Microsoft to view email logs.</p>
            <a
              href="/auth/microsoft/login?returnTo=/intranet/email-log"
              className="inline-flex rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white transition hover:bg-brandblack"
            >
            ...
            <section className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <span className="text-sm text-gray-600">
                  Current:{' '}
                  <strong className="text-gray-900">{currentEvent || 'No event set'}</strong>
                </span>
                        ...
                        <tr key={entry.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 whitespace-nowrap">
                            ...
                            <div className="text-xs text-gray-400">
                              {toLocalTimeStr(entry.timestamp)}
                            </div>
                          </td>
