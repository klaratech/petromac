...
- `public/flipbooks/` — Generated document bundles. **success-stories** is an image flipbook (WebP q80 pages since Jul 2026; `Flipbook.tsx` loads a ±4 window around the current spread, so a first view fetches ~5 images; manifest imported at build time via `src/features/flipbooks/manifests.ts`). **catalog** moved to an in-browser pdf.js viewer in Jul 2026 and uses a single-PDF scheme: each new catalog is compressed to <4 MB on ingest (Ghostscript) and linearized (qpdf), and that one `petromac-product-catalog.pdf` serves the viewer, the Download button, and emailed attachments — plus `search-index.json` (per-page text for the viewer's search box); no page images, no manifest, no separate email.pdf. Success-stories keeps its own compressed `email.pdf` (committed) since its source stays full-res for page rendering.
