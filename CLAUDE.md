...
- `public/flipbooks/` — Generated flipbook bundles, incl. `email.pdf` (committed). Pages are WebP (q80) since Jul 2026; `Flipbook.tsx` loads them in a ±4 window around the current spread, so a first view fetches ~5 images, not the whole book. Manifests are imported at build time via `src/features/flipbooks/manifests.ts` — a rebuilt bundle needs a commit + deploy to take effect.
