...
- **Map components**: `src/components/geo/` — `DrilldownMapCore` is the shared map; the public `/track-record` page is a server component (stat tiles + metadata from the build-time stats snapshot) whose only client island is `TrackRecordMap` (fetch + `next/dynamic` map import; its pre-fetch state is a crawlable track-record summary paragraph, not a spinner). The kiosk dashboard uses `DrilldownMapKiosk` as a wrapper
...
- **Homepage (v2, Jul 2026)**: hero with a typewriter headline (full text always in SSR HTML; animation is a hydration-time enhancement, reduced-motion-aware, runs once) over a seamless 14.9s background loop (`public/videos/hero/hero-loop.mp4` via `LazyVideo` — IO-gated so mobile/reduced-motion never download it); `ChallengeSelector` tabs; matched Hardware/Software bands (the software band hosts `AthenaTerminal`, a simulated terminal that types six lines on first view — also fully server-rendered); `ProofSection` stats from the build-time stats snapshot; contact section.
