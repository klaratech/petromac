...
- **Map components**: `src/components/geo/` — `DrilldownMapCore` is the shared map; the public `/track-record` page (redesigned Jul 2026, map-as-hero) is a server component: compact H1 + one-sentence intro carrying the headline numbers, then the map card with the three stats overlaid as compact tiles inside it (server-rendered; desktop top-left + pointer-transparent, mobile a slim row above the map; the card is `isolate` so in-map overlay z-indexes can't paint over the sticky header), then a build-time cumulative-deployments SVG chart (computed from a server-only import of the slim operations JSON), then a "Records & milestones" strip (verified facts linking to success stories/publications) with the Success Stories CTA. The only client island is `TrackRecordMap` (fetch + `next/dynamic` map import; pre-fetch state is a crawlable summary paragraph). Its data fetch is versioned with `operations_stats.json`'s `generatedAt` (see `fetchOperationsData`) so the map's dataset generation always matches the server-rendered numbers — kiosk callers keep the bare URL for stable SW cache keys. The kiosk dashboard uses `DrilldownMapKiosk` as a wrapper
