...
- **Kiosk**: OH/CH split — `/intranet/kiosk` (splash with Open Hole / Cased Hole buttons). OH → `/intranet/kiosk/lane?lane=oh` is now a pure attractor: a fullscreen subtitled-video playlist (dice intro sting + three narrated product clips) with native browser controls on the narrated clips and a small prev/next + dot strip at the bottom-centre that steps among the non-dice clips. The old right-edge product button strip (Formation Testing / High Deviation / PathFinder → `OverlayExperience`) was dropped in May 2026 — `OverlayExperience` still exists for `ProductlinesClient` but nothing on the lane wires it up anymore. CH → `/intranet/kiosk/ch` lands directly in `HelixExperience` (helix-subtitled video loop + HUD + Rocker corner badge — Rocker via `RockerExperience` as a sub-view). The old CH lane attractor was retired in May 2026 because it was a near-identical preview of the Helix video that the experience itself already plays; `?lane=ch` still works as a redirect to `/ch` for bookmarks. `/productlines?lane=oh|ch` (the `SystemModal` tile grid) is still around for direct links but is no longer in the main flow. Kiosk videos resolve via `useKioskVideo` — prefers `public/videos/kiosk-hd/` (1080p), falls back to `public/videos/transcoded/`. Native `controls` are exposed on the lane attractor (narrated clips only — dice stays chrome-free), the Helix video, and the OverlayExperience product video; mechanism clips inside `MechanismScreen` stay bare. Each experience HUD = `Mechanism · Case Studies`; Track Record + Success Stories live INSIDE `Case Studies` (first slide of the pager is the drill-down map, in-map link opens Success Stories as an inline takeover). `MechanismScreen` is a single-slide pager (annotated diagrams / video / comparison) with a `Specifications` button that opens a spec-sheet modal sourced from `deviceSpecs.ts`.
